...
if(liveRecording == true)
    ...
    chirpConfigurationFileName = '3D People counting_MSU.cfg';
%{
    fhistFilePath = 'D:\testVectors\2020_03_25_LivingRoom_ODS_CM\fHists_RT';
    fhistFileName = 'fHist_s02_a08';   
%}
    %fhistFilePath = 'D:\testVectors\2020_08_26_NitinVectors_ODS_CM\circle_walk_1m_off_boresight_110_tilt';
    %fhistFileName = 'fHistRT'; 
    
    fhistFilePath = 'E:\Data\PointCloud\Unlabeled';
    fhistFileName = 'fHistRT_0004';
end
...
if(strcmp(sceneRun,'PerfTestArea'))
    scene.maxPos = [[-4 4] [0 8] [-1 3]]; %xmin,xmax, ymin,ymax, zmin,zmax
    scene.sensorPos = [0, 0, 2.0]; %x,y, and z of sensor
    scene.numberOfBoundaryBoxes = 1;
    scene.boundaryBox(1,:) = [-4 0 -0.5 8 8 3.5]; %x,y,z (left,near,bottom), width, depth, height
    %    scene.boundaryBox(1,:) = [-1.5 2 -1 3.8 4.8 4]; %x,y,z (left,near,bottom), width, depth, height
    scene.numberOfStaticBoxes = 1;
    scene.staticBox(1,:) = scene.boundaryBox(1,:) + [0.5 0.5 0 -1 -1 -0.5]; %x,y,z (left,near,bottom), width, depth, height
    %    scene.staticBox(1,:) = [-1.4 2 -1 3.4 4.4 4]; %x,y,z (left,near,bottom), width, depth, height
    scene.numberOfTargetBoxes = 1;
    scene.targetBox(1,:) = [-3 0 0 6 6 2]; %x,y,z (left,near, bottom), width, depth, height
    scene.targetBox(2,:) = [0 0 0 0 0 0]; %x,y,z (left,near, bottom), width, depth, height
    scene.azimuthTilt = 0*pi/180;
    scene.elevationTilt = 10*pi/180;
end
...
set(figHandle,'Name','Texas Instruments - Indoor People Counting V3.0','NumberTitle','off')    
    ...
    if(strcmp(figureTitles{iFig},'Gating and Association'))
        if(showGatingInCartesian == 1)
            ...
            if(dSize == 3)
                ...
                zlabel('Z, m');
                axis equal;
                axis(scene.maxPos);%axis([[-3 3] [0 5] [0 3]]);
                %view(0,0);
                view(45,45);
            else
                axis([scene.maxPos(1:4) [-2*sensor.maxRadialVelocity +2*sensor.maxRadialVelocity]]);
...
if(liveRecording == false)
    ...
    matlabFileName = [fhistFilePath,'\',fhistFileName, '.mat'];
...
while(1)
    while(lostSync == 0)
        ...
        if(liveRecording == true)
            ...
            if(length(fHist) > 25)
                result = FeatureParsingGUI(fHist(frameNum-20:frameNum-1));
                ...
                if (features)
                    for i = 1:length(features(:,1))
                        [guess, other] = predict(model, features(i,:));
                        target_guesses{tids(i)+1,1} = [target_guesses{tids(i)+1,1}, guess];
                    end
                end
            end
...
%}                
        ...
        for n=1:numTargets
            ...
            if(numTargets > 0)
                ...
                if(length(target_guesses) >= n)
                    
                    target_label = mode(target_guesses{tarid+1,1});

                    if (target_label == -1)
                        display_label = "N/A";
                    elseif (target_label == 0)
                        display_label = "Other";
                    elseif (target_label  ==  1)
                        display_label = "Human";
                    elseif (target_label == 2)
                        display_label = "Cat";
                    else
            ...
            if isempty(trackingHist(tid).hLabel)
                ...
                if ~ishandle(trackingHist(tid).hLabel)
                    ...
                    set(trackingHist(tid).hLabel,'Position',[tPos(1)+labelOffset,  tPos(2)+labelOffset, tPos(3)+labelOffset], 'String', [strcat('TID ', num2str(tid-1)),' : '+display_label]);
                end
            end
